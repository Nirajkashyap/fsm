...
//! Activity Gateway.
//!
//! Rust counterpart of ../typescript/cli.ts and ../python/cli.py: calls
...
fn parse_args() -> Args {
    ...
    let folder_path = folder_path.unwrap_or_else(|| {
        eprintln!("--folder-path is required");
        print_help();
        process::exit(1);
    });
...
fn main() {
    ...
    let results = validate_async_operation_from_folders_rust(&args.folder_path, &args.skip_dirs);
    println!(
