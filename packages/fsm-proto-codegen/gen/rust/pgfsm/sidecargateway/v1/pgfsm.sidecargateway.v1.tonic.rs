...
pub mod sidecar_gateway_service_client {
    #![allow(
        unused_variables,
        dead_code,
        missing_docs,
    ...
    #[derive(Debug, Clone)]
    pub struct SidecarGatewayServiceClient<T> {
        inner: tonic::client::Grpc<T>,
    }
    impl SidecarGatewayServiceClient<tonic::transport::Channel> {
    ...
    impl<T> SidecarGatewayServiceClient<T>
    where
        ...
        <T::ResponseBody as Body>::Error: Into<StdError> + std::marker::Send,
    {
        ...
        pub fn with_interceptor<F>(
            inner: T,
            interceptor: F,
        ) -> SidecarGatewayServiceClient<InterceptedService<T, F>>
        where
        ...
        {
            SidecarGatewayServiceClient::new(InterceptedService::new(inner, interceptor))
        }
...
pub mod sidecar_gateway_service_server {
    #![allow(
        unused_variables,
        dead_code,
        missing_docs,
    ...
    impl<T> SidecarGatewayServiceServer<T> {
        pub fn new(inner: T) -> Self {
    ...
    for SidecarGatewayServiceServer<T>
    where
    ...
    impl<T> Clone for SidecarGatewayServiceServer<T> {
        fn clone(&self) -> Self {
            let inner = self.inner.clone();
            Self {
                inner,
    ...
    pub const SERVICE_NAME: &str = "pgfsm.sidecargateway.v1.SidecarGatewayService";
    impl<T> tonic::server::NamedService for SidecarGatewayServiceServer<T> {
