...
//!
//! Rust port of the gateway's sidecar/protocol.ts. Wire shapes must match
//! that file byte-for-byte (same JSON key names, same framing) since a Rust
//! worker process and the TypeScript-hosted gateway talk this protocol to
...
/// generated registry identity fields — `actor_key()` is what the gateway
/// routes on.
#[derive(Debug, Serialize, Deserialize, Clone)]
