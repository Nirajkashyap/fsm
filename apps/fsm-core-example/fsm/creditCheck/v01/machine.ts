...
export const machine = createMachine({
  ...
  context: ({ input }: any) => ({
    ...
    GavUnionScore: input?.GavUnionScore ?? 0,
    EquiGavinScore: input?.EquiGavinScore ?? 0,
    GavperianScore: input?.GavperianScore ?? 0,
    ErrorMessage: input?.ErrorMessage ?? "",
    MiddleScore: input?.MiddleScore ?? 0,
    InterestRateOptions: input?.InterestRateOptions ?? [],
  }),
      ...
      states: {
        ...
        CheckingCreditScores: {
          ...
          onDone: [
            ...
            {
              target: "Entering Information",
              actions: [{ type: "assignCreditScoreError" }],
            },
          ],
        },
          ...
          states: {
            DeterminingMiddleScore: {
              invoke: {
                ...
                }: any) => [EquiGavinScore, GavUnionScore, GavperianScore],
                src: "determineMiddleScore",
                id: "scoreDeterminationActor",
                onDone: [
                  {
                    actions: [{ type: "assignMiddleScore" }, {
            ...
            FetchingRates: {
              invoke: {
                input: ({ context: { MiddleScore } }: any) => MiddleScore,
