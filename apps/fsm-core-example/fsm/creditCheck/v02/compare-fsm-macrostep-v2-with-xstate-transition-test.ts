...
Deno.test({
  name:
    "Journey 1 — initialTransition_event: macrostep_v2 state matches xstate initialTransition",
...
Deno.test({
  name:
    "Journey 2 — Submit: macrostep_v2 state matches xstate transition after Submit",
  sanitizeResources: false,
  sanitizeOps: false,
  async fn() {
    const pool = new Pool({ connectionString: Deno.env.get("DATABASE_URL") });
    ...
    try {
      ...
      const resolved_state_value = await resolveStateValue(
        dbDeps,
        replaceSpacesWithUnderscores(resolvedInitialState.value),
        fsm_name,
        fsm_version,
      );
...
Deno.test({
  name:
    "Journey 3 — xstate.done.actor (Verifying Credentials): macrostep_v2 state and actions match xstate",
  sanitizeResources: false,
  sanitizeOps: false,
  async fn() {
    const pool = new Pool({ connectionString: Deno.env.get("DATABASE_URL") });
    ...
    try {
      ...
      const [afterSubmit] = transition(machineConfig, resolvedInitialState, {
        type: "Submit",
        payload: {},
      });
