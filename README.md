...
the directory doesn't exist yet). Each connects to the gateway's
...
# Python — also runs from the repo root as-is (the script's own directory is
# always on sys.path), but needs its deps installed first:
#   pip install -r apps/fsm-core-example/worker-sdk-generated/python/requirements.txt
python3 apps/fsm-core-example/worker-sdk-generated/python/cli.py start \
  --gateway-socket /tmp/pgfsm-activity-gateway-workers.sock
